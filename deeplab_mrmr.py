...
class Config:
    ...
    IMAGE_HEIGHT = 128
    IMAGE_WIDTH = 128
    # Define the specific bands you want to use
    SELECTED_BANDS = [
        1,
        88,
        2,
...
def create_deeplabv3(input_shape, num_classes):
    ...
    # Rest of the model creation code remains the same
...
def save_model_outputs(model, history, predictions_test, masks_test, f1, test_loss=None):
    ...
    folder_name = f"deeplabv3_full_score_{timestamp}"
    ...
    with open(os.path.join(folder_name, 'selected_bands.txt'), 'w') as f:
        f.write(f"Selected bands: {Config.SELECTED_BANDS}\n")
...
def train_on_dataset():
    ...
    # Load dataset with selected bands
