...
# These are example bands - modify based on your needs
...
def load_all_images(image_dir, label_dir, selected_bands):
    """Load all image and label pairs from the directories using only selected bands"""
    image_files = sorted([f for f in os.listdir(image_dir) if f.endswith('.tif')])
    all_images = []
    all_labels = []

    print(f"Found {len(image_files)} image files")
    print(f"Using {len(selected_bands)} selected bands: {selected_bands}")
...
def main():
    ...
    output_dir = f'unet_fullbands_{timestamp}'
    ...
    try:
        ...
        # Load images and labels with selected bands
        print("Loading images and labels...")
        images, labels = load_all_images(image_dir, label_dir, SELECTED_BANDS)
